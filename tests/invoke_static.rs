...
fn invoke_static_simple_no_args() -> Result<()> {
    ...
    vm.run("test_data/Simple", "no_args", vec![]);
    /*
...
fn invoke_static_simple_add() -> Result<()> {
    let mut vm = VirtualMachine::new();
    vm.register_class(ClassReader::from_file("./tests/test_data/Simple.class")?);

    let return_value = vm.invoke_static_entry_point("test_data/Simple", "add", vec![Int(1), Int(5)]);
...
fn invoke_static_simple_add_long() -> Result<()> {
    ...
    let return_value = vm.invoke_static_entry_point("test_data/Simple", "add_long", vec![Long(1), Long(5)]);
...
fn invoke_static_nested() -> Result<()> {
    ...
    let return_value = vm.invoke_static_entry_point("test_data/Simple", "add_nested", vec![Int(1), Int(5)]);
    assert_eq!(return_value, Some(Int(6)));
    Ok(())
}*/