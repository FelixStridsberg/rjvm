...
fn invoke_static_simple_add() {
    assert_eq!(
        java::run_method_args("test_data/Simple", "add", vec![Int(1), Int(5)]),
        Int(6)
    );
}
...
fn invoke_static_simple_add_long() {
    assert_eq!(
        java::run_method_args("test_data/Simple", "add_long", vec![Long(1), Long(5)]),
        Long(6)
    );
}
...
fn invoke_static_nested() {
    assert_eq!(
        java::run_method_args("test_data/Simple", "add_nested", vec![Int(1), Int(5)]),
        Int(6)
    );
}
